<script lang="ts">
  ...
  let linkedOrganizationId = $state('');
  let website = $state('');
  let address = $state('');
  ...
  async function loadContact() {
    ...
    try {
      ...
      dealsLoading = true;
      await Promise.all([
        organizationStore.loadOrganizations().catch(() => undefined),
  ...
  function populateForm(c: Contact) {
    ...
    linkedOrganizationId = c.organizationId ?? '';
    website      = c.website ?? '';
    address      = c.address ?? '';
    contactType  = c.type;
    notes        = c.notes ?? '';
  ...
  function handleOrganizationLinkChange() {
    const selected = organizationStore.organizations.find((org) => org.id === linkedOrganizationId);
    if (selected) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      let updated = await contactStore.updateContact(contact.id, payload);
      const nextOrganizationId = linkedOrganizationId || null;
      if (updated.organizationId !== nextOrganizationId) {
        updated = await contactStore.linkContactToOrganization(contact.id, nextOrganizationId);
      }
      contact = updated;
        ...
        <section class="card detail-fields" aria-labelledby="fields-heading">
          ...
          <div class="card-body">
            <div class="fields-grid">
              ...
              <div class="field-group">
                <label class="field-label" for="linkedOrganization">{t('organizations.linkedOrganization')}</label>
                <select
                  id="linkedOrganization"
                  class="select"
                  bind:value={linkedOrganizationId}
                  onchange={handleOrganizationLinkChange}
                >
                  <option value="">{t('organizations.noLinkedOrganization')}</option>
