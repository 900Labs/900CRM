...
function toUiStage(stage: string): DealStage | null {
  const key = stage.toLowerCase().trim();
  const mapped = UI_STAGE_MAP[key];
  if (mapped) {
    return mapped;
  }
  if (import.meta.env.DEV && typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('deals-stage-fallback', {
        detail: `Unknown backend stage "${stage}" excluded from typed stage mapping`,
      })
    );
  }
  return null;
...
function mapDeal(deal: BackendDeal): Deal | null {
  const stage = toUiStage(deal.stage);
  if (!stage) {
    return null;
  }
...
function requireMappedDeal(deal: BackendDeal, command: string): Deal {
  const mapped = mapDeal(deal);
  if (mapped) {
    return mapped;
  }
  throw new Error(`Unsupported deal stage returned by ${command}: "${deal.stage}"`);
}
...
export async function updateDeal(id: string, data: UpdateDealPayload): Promise<Deal> {
  ...
  return requireMappedDeal(deal, 'update_deal');
}
...
export async function moveDealStage(id: string, stage: DealStage): Promise<Deal> {
  ...
  return requireMappedDeal(deal, 'move_deal_stage');
}
...
export async function getPipelineSummary(): Promise<PipelineSummary> {
  ...
  for (const row of summary) {
    ...
    if (!stage) {
      throw new Error(`Unsupported deal stage returned by get_pipeline_summary: "${row.stage}"`);
    }
    byStage[stage] = {
      count: row.count ?? 0,
