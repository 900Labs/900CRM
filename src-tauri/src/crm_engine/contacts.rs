...
pub fn find_duplicate_candidates(
    ...
    let first = input.first_name.as_deref().unwrap_or("").trim().to_lowercase();
    let last = input.last_name.as_deref().unwrap_or("").trim().to_lowercase();
...
pub fn merge_contacts(
    ...
    let merged_email = if target.email.is_empty() { &source.email } else { &target.email };
    let merged_phone = if target.phone.is_empty() { &source.phone } else { &target.phone };
    let merged_address = if target.address.is_empty() { &source.address } else { &target.address };
    let merged_city = if target.city.is_empty() { &source.city } else { &target.city };
    let merged_country = if target.country.is_empty() { &source.country } else { &target.country };
    let merged_org_name = if target.org_name.is_empty() { &source.org_name } else { &target.org_name };
