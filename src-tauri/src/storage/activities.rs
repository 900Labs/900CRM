...
pub fn create_activity(
    ...
    conn.execute(
        ...
        params![
            ...
            contact_id, deal_id, now, now, device_id
        ],
    )?;
...
///
/// Returns [`CrmError::Database`] on SQL failure.
pub fn list_activities_for_contact(conn: &Connection, contact_id: &str) -> CrmResult<Vec<Activity>> {
    let mut stmt = conn.prepare(
        r#"
        SELECT id, activity_type, title, description, due_date, completed,
               contact_id, deal_id, created_at, updated_at, deleted_at, device_id
...
pub fn update_activity(
    ...
    conn.execute(
        ...
        params![
            ...
            completed.map(|c| c as i32).unwrap_or(current.completed as i32),
