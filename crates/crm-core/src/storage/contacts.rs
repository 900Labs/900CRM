...
pub fn list_contacts(
    ...
    let (total, contacts) = match &params.filter_type {
        Some(ft) => {
            ...
            let rows = stmt.query_map(params![limit, offset, ft], |row| row_to_contact(row))?;
        ...
        None => {
            let total: u32 = conn
                ...
                .unwrap_or(0);
            let mut stmt = conn.prepare(&sql_list)?;
            let rows = stmt.query_map(params![limit, offset], |row| row_to_contact(row))?;
            let contacts: Vec<Contact> = rows.filter_map(|r| r.ok()).collect();
            (total, contacts)
        }
    };
...
fn search_contacts_paged(
    ...
    let rows = stmt.query_map(params![fts_query, limit, offset], |row| row_to_contact(row))?;
    let contacts: Vec<Contact> = rows.filter_map(|r| r.ok()).collect();
