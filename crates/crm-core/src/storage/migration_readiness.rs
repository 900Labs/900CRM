...
pub fn get_normalization_migration_readiness(
    ...
    Ok(NormalizationMigrationReadinessCounts {
        ...
        contacts_with_invalid_organization_links: count_contacts_with_invalid_organization_links(
            conn,
        )?,
...
fn count_contacts_with_invalid_organization_links(conn: &Connection) -> CrmResult<i64> {
    conn.query_row(
        r#"
        SELECT COUNT(*)
        FROM contacts AS c
        WHERE c.deleted_at IS NULL
          AND (
            (
              TRIM(COALESCE(c.organization_id, '')) <> ''
              AND NOT EXISTS (
                SELECT 1
