...
[![Languages: 6](https://img.shields.io/badge/Languages-6-brightgreen.svg)](https://github.com/900-labs/900crm#language-support)
...
Before opening a pull request, run these checks in order:
...
Pass criteria:
- `npm run check:tooling`: Svelte/Vite/Vitest tools resolve from this repository
- `npm run check`: no Svelte or TypeScript errors
- `npm run test`: frontend API tests run through Vitest
- `npm run lint`: no ESLint errors
- `cargo test`: all Rust tests pass
- `cargo check`: backend compiles cleanly
- `npm run build`: frontend production bundle builds successfully
...
│       ├── src-tauri/            # Thin Tauri shell and IPC commands
│       ├── package.json
│       ├── svelte.config.js
│       ├── tsconfig.json
│       └── vite.config.ts
...
- [ ] Additional languages (Portuguese, Vietnamese, Hausa, Bengali)
